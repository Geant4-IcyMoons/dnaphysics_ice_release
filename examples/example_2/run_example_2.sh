...
Options:
  ...
  --output-dir PATH   Directory for example_2.root
...
echo "Running example_2 -> ${OUTPUT_DIR}/example_2.root"
(
  cd "${OUTPUT_DIR}"
  DNA_PHYSICS="${PHYSICS}" \
  DNA_ROOT_BASENAME="example_2" \
  DNA_NTUPLE_FILES=0 \
  DNA_NTUPLE_STRINGS=1 \
  "${BINARY}" "${EXAMPLE_DIR}/example_2.mac" "${THREADS}"
)
