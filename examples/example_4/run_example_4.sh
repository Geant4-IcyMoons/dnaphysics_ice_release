...
WVALUE_FILE="example_4_wvalue.txt"
...
Options:
  ...
  --output-dir PATH   Directory for example_4.root and W-value text output
...
echo "Running example_4 -> ${OUTPUT_DIR}/example_4.root"
echo "W-value rows will be appended to ${OUTPUT_DIR}/${WVALUE_FILE}"
(
  cd "${OUTPUT_DIR}"
  DNA_PHYSICS="ice_am" \
  DNA_ROOT_BASENAME="example_4" \
  DNA_WVALUE_FILE="${WVALUE_FILE}" \
  DNA_NTUPLE_FILES=0 \
  DNA_NTUPLE_STRINGS=1 \
  "${BINARY}" "${EXAMPLE_DIR}/example_4.mac" 12
)
