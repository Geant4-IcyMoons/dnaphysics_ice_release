...
Options:
  ...
  --output-dir PATH   Directory for example_1.root
  --threads N         Number of Geant4 threads (default: 1)
...
echo "Running example_1 -> ${OUTPUT_DIR}/example_1.root"
(
  cd "${OUTPUT_DIR}"
  DNA_PHYSICS="${PHYSICS}" \
  DNA_ROOT_BASENAME="example_1" \
  DNA_NTUPLE_FILES=0 \
  DNA_NTUPLE_STRINGS=1 \
  "${BINARY}" "${EXAMPLE_DIR}/example_1.mac" "${THREADS}"
)
