...
WVALUE_FILE="example_6_wvalue.txt"
...
Options:
  ...
  --output-dir PATH   Directory for example_6.root and W-value text output
...
echo "Running example_6 -> ${OUTPUT_DIR}/example_6.root"
echo "W-value rows will be appended to ${OUTPUT_DIR}/${WVALUE_FILE}"
(
  cd "${OUTPUT_DIR}"
  DNA_PHYSICS="water" \
  DNA_ROOT_BASENAME="example_6" \
  DNA_WVALUE_FILE="${WVALUE_FILE}" \
  DNA_NTUPLE_FILES=0 \
  DNA_NTUPLE_STRINGS=1 \
  "${BINARY}" "${EXAMPLE_DIR}/example_6.mac" 12
)
